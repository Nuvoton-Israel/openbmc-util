...
unsigned char *rbuf_ptr =NULL;


int total_write =0;
void MainWindow::handle_write(char *qbuffer, unsigned long receive)
{
    ...
    unsigned long total_write_this_time=receive;
    ...
    while (total_write_this_time > 0)
    {
        ...
        if (!allowWrite) {
            ...
            qInfo("total_write times:%d (%ld)(from:%lld)",total_write,total_write_this_time,write_from);
            bWritten = ::write(fh,buffer,nb);
            if(nb != bWritten){
                qInfo("Failed to write %ld bytes  (ret:%d)", nb, bWritten);
                goto error;
            }
...
void MainWindow::handle_read(struct nbd_request *req)
{
    ...
    len = ntohl(req->len);
    //qInfo("------------------------REQ from 0x%lx len %ld", from, len);

    while (len > 0) {
        nb = len;
        if(nb>rbuf_size)
            nb = rbuf_size;
        lseek(fh,from,SEEK_SET );
        dummy = read(fh,rbuffer,nb);
        if(dummy!=nb){
            qInfo("Failed to read:%lu",dummy);
            goto error;
        }
        rbuf_ptr = &rbuffer[0];
...
int MainWindow::handle_request(struct nbd_request req)
{
    ...
    if (command != NBD_CMD_DISC && check_length(&req, fsize) < 0){
...
int MainWindow::opendevice(void){
    ...
    if(connection_flag==0){
     ...
     if (allowWrite){
         fh=::open(dev_name,O_RDWR);
         if (fh < 0){
             fh=::open(dev_name,O_RDONLY);
             allowWrite = false;
             qInfo("Open file %s as readonly, due to non-writable", dev_name);
             QMessageBox::information(this, tr("Info"), "Open deivce ReadOnly");
         }
     ...
     if (fh < 0)
     {
         ...
         lseek(fh, (size_t)0, SEEK_CUR);
         fsize = lseek(fh, (size_t)0, SEEK_END);
         qInfo("VolumeLength of file: %lld",fsize);
     }
...
int MainWindow::nbd_server_start(int argc, char *argv[])
{
    ...
    else
    {
        ...
        qInfo("Error opening file: %s",dev_name);
        QString status = QString("Open file %1 Error: %2").arg(dev_name).arg(strerror(errno));
    ...
    QUrl url;
    url.setUrl(ws_srv);
    qInfo("Connect to websocket server: %s",ws_srv);
    Connectedto(url);
