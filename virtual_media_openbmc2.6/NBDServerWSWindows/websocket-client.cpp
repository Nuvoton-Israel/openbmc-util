...
void MainWindow::onSslMessageReceived(void)
{
    ...
    while (sslSocket->canReadLine())
    {
        ...
        if(line.startsWith("Set-Cookie: SESSION=",Qt::CaseInsensitive)){
            int lastindex = line.lastIndexOf(';');//"
            int firstindex = line.indexOf(";",12,Qt::CaseInsensitive);
            //qInfo("read line(%s)",line.toUtf8().constData());
            SID = line.mid(12,firstindex-12);//
            qInfo("1-SID(%s)",SID.toUtf8().constData());
        ...
        //qInfo("Got(%s)(%d)",line.toUtf8().constData(),line.count());
        //qInfo("readLine-------");
    }
    //qInfo("login_get_sid(%d),response_has_body(%d),connected_to_wss(%d)",login_get_sid,response_has_body,connected_to_wss);
    if(login_get_sid  && !connected_to_wss){//&& !response_has_body
        connected_to_wss=1;
        qInfo("ConnectedtoWS++++++");
        ConnectedtoWS();
    }
    ...
    while(read_len<got_len){
        ...
        case 3://required 4 bytes
            ...
            if(opt==NBD_OPT_EXPORT_NAME){
                ...
                read_len+=4;
                qInfo("unknow_opt_len-1:%d",unknow_opt_len);
                unknow_opt_len = ntohl(unknow_opt_len);
                qInfo("unknow_opt_len-2:%d",unknow_opt_len);
