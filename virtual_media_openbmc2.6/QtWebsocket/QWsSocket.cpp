...
void QWsSocket::initTcpSocket()
{
    ...
    //QObject::connect(tcpSocket, SIGNAL(readyRead()),this, LOT(processLogin()), Qt::UniqueConnection);
...
void QWsSocket::onSslErrors(const QList<QSslError> &errors)
{
    //Q_UNUSED(errors);
    ...
    for (int i=0, sz=errors.size(); i<sz; i++)
    {
        ...
        qInfo("onSslErrors in QWsSocket:%s",errorString.toStdString().c_str());
    }
}

void QWsSocket::PrintSocketState(){
    QAbstractSocket::SocketState SocketState = QAbstractSocket::SocketState();
    switch (SocketState)
    {
        case QAbstractSocket::HostLookupState:
        {
            qInfo("QAbstractSocket::HostLookupState");
            break;
        }
        case QAbstractSocket::ConnectingState:
        {
            qInfo("QAbstractSocket::ConnectingState");
            break;
        }
        case QAbstractSocket::ConnectedState:
        {
            qInfo("QAbstractSocket::ConnectedState");
            break;
        }
        case QAbstractSocket::ClosingState:
        {
            qInfo("QAbstractSocket::ClosingState");
            break;
        }
        case QAbstractSocket::UnconnectedState:
        {
            qInfo("QAbstractSocket::UnconnectedState--");
            break;
        }
        default:
            qInfo("QAbstractSocket::default");
            break;
    }
}

void QWsSocket::connectToWSSHost(const QString& hostName, QSslSocket *sslSocket_from_client, quint16 port, OpenMode mode)
{
    qInfo("In connectToWSSHost(%s)",hostName.toStdString().c_str());
    PrintSocketState();
    ...
    qInfo("Good::the socket is in the Unconnected state");

    // trim the hostname
    _hostName = hostName.trimmed();
    qInfo("_hostName-1:%s",_hostName.toStdString().c_str());
    _hostAddress = QHostAddress("192.168.1.127");
    qInfo("_hostName-2:%s",_hostName.toStdString().c_str());
    _host = "192.168.1.127";
#if 1
    ...
    tcpSocket = sslSocket_from_client;
    sslSocket = sslSocket_from_client;
    initTcpSocket();
    qInfo("Before onEncrypted");
    onEncrypted();
...
void QWsSocket::connectToHost(const QString& hostName, quint16 port, OpenMode mode)
{
	...
	// trim the hostname
	_hostName = hostName.trimmed();
    qInfo("_hostName:%s",_hostName.toStdString().c_str());
	...
	// remove the websocket URI scheme for TCP connection
	_host = QString(_hostName).remove(regExpUriStart);
    qInfo("_host:%s",_host.toStdString().c_str());

	_hostPort = port;
     qInfo("_hostPort:%d",_hostPort);
	// check localhost uri
	if (_host.contains(regExpLocalhostUri))
	{
		_hostAddress = QHostAddress::LocalHost;
         qInfo("_hostAddress-1:%s",_hostAddress.toString().toStdString().c_str());
	}
	// check IPv4 URI
	// TODO IPv6
	else if (_host.contains(QRegExp(QWsSocket::regExpIPv4)))
	{
		_hostAddress = QHostAddress(_host);
        qInfo("_hostAddress-2:%s",_hostAddress.toString().toStdString().c_str());
	}
	// from hostName
	else
	{
		...
		{
			_hostAddress = hostAddresses.first();
            qInfo("_hostAddress-3:%s",_hostAddress.toString().toStdString().c_str());
		}
		else
		{
            qInfo("_hostAddress return");
			return;
		}
	}
	...
	if (_secured)
	{
        ...
        qInfo("connectToHostEncrypted(%s)",_host.toUtf8().constData());

        sslSocket->setPeerVerifyMode(QSslSocket::VerifyNone);
        //sslSocket->ignoreSslErrors();//Medad:this function is not working, if I invoke it at this point
...
void QWsSocket::processHandshake()
{
	...
	{
        qInfo("return ConnectionRefusedError--1");
	...
	{
        qInfo("return readStarted complete");
		return;
	}
	...
	{
        qInfo("return  mandatory params are not setted");
		emit error(QAbstractSocket::ConnectionRefusedError);
		return;
	}
...
void QWsSocket::onEncrypted()
{
    qInfo("onEncrypted++--++");
    if (_wsMode == WsClientMode){
        qInfo("Call startHandshake()");
        startHandshake();
    }else
        qInfo("Do NOT Call startHandshake()");
}

void QWsSocket::finishedSlot(QNetworkReply *reply)
{
...
{
    qInfo("In startHandshake");
	if (_version == WS_V13)
	{
		key = QWsSocket::generateNonce();
        QString handshake = composeOpeningHandShakeV13(WSPath(), _host, key);
        qInfo("Handshake first MSG-WS_V13:(%s)",handshake.toUtf8().constData());
	...
	else if (_version == WS_V0)
	{
        ...
        QString handshake = composeOpeningHandShakeV0(WSPath(), _host, key1, key2, key3);
        qInfo("Handshake first MSG-WS_V0:(%s)",handshake.toUtf8().constData());
        tcpSocket->write(handshake.toUtf8());
	}
	else // support more version soon
	{
        qInfo("This protocol version in not implemented");
...
{
	QString hs;
    qInfo("composeOpeningHandShakeV13()");
    ...
    hs += QLatin1String("Pragma: no-cache\r\n");
    hs += QLatin1String("Cache-Control: no-cache\r\n");
    hs += QString("Accept-Encoding: gzip, deflate, br\r\n");
    hs += QString("Accept-Language: en-US,en;q=0.9,zh-TW;q=0.8,zh;q=0.7\r\n");
    hs += QLatin1String("User-Agent: User-Agent: Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/70.0.3538.110 Safari/537.36\r\n");
    hs += QString("Cookie: %1\r\n").arg(SIDfromHTTP());
    hs += QString("Sec-WebSocket-Key: %1\r\n").arg(QLatin1String(key));
	hs += QLatin1String("Sec-WebSocket-Version: 13\r\n");
    hs += QLatin1String("Sec-WebSocket-Extensions: permessage-deflate; client_max_windows_bits\r\n");
...
void QWsSocket::setSIDfromHTTP(QString s)
{
    _SIDfromHTTP = s;
}

void QWsSocket::setWSPath(QString wp)
{
    _WSPath = wp;
}
...
QString QWsSocket::SIDfromHTTP()
{
    return _SIDfromHTTP;
}

QString QWsSocket::WSPath()
{
    return _WSPath;
}
} // namespace QtWebsocket
